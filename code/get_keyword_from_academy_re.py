...
def path_check(ch_file, num_check, download_path, added_name):
    if num_check != 0:
        while not os.path.exists(download_path + '/' + added_name + ch_file + str(datetime.today().day) + ' (' + str(num_check) + ').xls'):
            time.sleep(1)
        num_check = num_check + 1
    else:
        while not os.path.exists(download_path + '/' + added_name + ch_file + str(datetime.today().day) + '.xls'):
            time.sleep(1)
        num_check = num_check + 1
