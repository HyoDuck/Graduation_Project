...
import os
import Start_with_this

class MyWindow(QWidget):
    start_date = '2018.01.01'         # 시작일
    end_date = '2018.10.30'           # 마감일

    start_date_for_download = '2018-01-01'
    end_date_for_download = '2018-10-30'

    navertrend_path = 'C:/data/Keywords'    # 네이버 트렌드를 다운받을 경로
    stock_price_xlsx_path = 'C:/data/2000x2000.xlsx'          # 네이버에서 받아온 종가엑셀
    xlsx_path = 'C:/data/corr.xlsx'              # 상관관계 엑셀(corr.xlsx)
    ...
    def setupUI(self):
        ...
        pushButton10.clicked.connect(self.pushButton_draw_graph1)
    ...
    def button_download(self):
        print("download")
        download_stock_info.downloading()  # 종가데이터 다운로드
    ...
    def pushButton_setting(self):
        ...
        self.start_date_for_download = str(self.spinBox1.value()) + '-' + start_month_set + '-' + start_day_set
        self.end_date_for_download = str(self.spinBox4.value()) + '-' + end_month_set + '-' + end_day_set
    ...
    def pushButton_start(self):
        ...
        download_stock_info.downloading()       # 종가데이터 다운로드
        # 상관관계 엑셀 생성
        v_test.corr(excel=os.path.dirname( os.path.abspath( __file__ ) ) + 'price_list.xlsx', start=self.start_date, end=self.end_date)
        # 클러스터링 이후 키워드 추출 시작
        Start_with_this.all_in_one(start_date=self.start_date, end_date=self.end_date,
                                   excel_path=os.path.dirname( os.path.abspath( __file__ ) ) + 'corr.xlsx', max_min=1.5)

        v_test.visualization(start=self.start_date, end=self.end_date, excel='', mat='')        # 전달인자 없이 바로 호출하도록 수정하였습니다.

    def pushButton_draw_graph1(self):       # 키워드 없이 그리기
        print("graph1")
        v_test.visualization(start=self.start_date, end=self.end_date, mat=self.xlsx_path, excel=self.stock_price_xlsx_path)
    ...
    def pushButon_keywords1(self):      # 소셜매트릭스 다운로드
        print("social matrix")
        Get_keywords.Keywords_academy_only_ver(start_date=self.start_date_for_download, end_date=self.end_date_for_download, excel_path=self.xlsx_path, max_min=1.5)
    ...
    def pushButton_NaverTrend(self):    # 네이버 트렌드 다운로드를 이용하여 소셜매트릭스에서 가져온 키워드들을 추출합니다.
        ...
        return_without_download.make_without_download(default_path=self.default_path,
                                                      start_date=self.start_date_for_download, end_date=self.end_date_for_download,
                                                      use_navertrend=True, use_downloaded_navertrend=True)
    ...
    def pushButton_make_keywords_xlsx(self):
        ...
        return_without_download.make_without_download(default_path=self.default_path,
                                                      ...
                                                      use_navertrend=False, use_downloaded_navertrend=True)
